...
Перем ПараметрыПодключенияКБазе;
Перем РежимОтладки;
...
Функция Инициализировать()
	...
	УстановитьПараметрыБлокировкиСеансов();
	
	ПараметрыРезервногоКопирования = Новый Структура;
	УстановитьПараметрыРезервногоКопирования();
	
	ПараметрыПодключенияКБазе = Новый Структура;
	УстановитьПараметрыПодключенияКбазе();
...
//
Процедура УстановитьПараметрыБлокировкиСеансов(БлокироватьСеансыИБ = Истина, ИспользованиеИБ = Истина, ИнтервалОжидания = 900) Экспорт
	
	ПараметрыБлокировкиСеансов.Вставить("БлокироватьСеансыИБ", БлокироватьСеансыИБ);
	ПараметрыБлокировкиСеансов.Вставить("ИспользованиеИБ", ИспользованиеИБ);
	ПараметрыБлокировкиСеансов.Вставить("ИнтервалОжидания", ИнтервалОжидания);
	
	Лог.Отладка(НСтр("ru = 'Установлены параметры блокировки сеансов:'"));
	Для Каждого Параметр Из ПараметрыБлокировкиСеансов Цикл
		Лог.Отладка(СтрШаблон(НСтр("ru = '	%1 - ""%2"";'"), Параметр.Ключ, Параметр.Значение));
	КонецЦикла;
	
КонецПроцедуры

// Устанавливает значения параметров подключения к базе
//
// Параметры:
//   ИмяКоннектора - Строка - Имя коннектора для подключения к базе
//
Процедура УстановитьПараметрыПодключенияКБазе(ИмяКоннектора = "") Экспорт
	
	ПараметрыПодключенияКБазе.Вставить("ИмяКоннектора", ИмяКоннектора);
	
	Лог.Отладка("Установлены параметры подключения к базе:");
	Для Каждого Параметр Из ПараметрыПодключенияКБазе Цикл
		Лог.Отладка(СтрШаблон("%1 - %2", Параметр.Ключ, Параметр.Значение));
	КонецЦикла;
...
Функция ПолучитьПараметры(Параметр) Экспорт
	...
	ИначеЕсли Параметр = "ПараметрыБлокировкиСеансов" Тогда
...
Процедура УстановитьРежимОтладки(Включить = Истина) Экспорт
	...
	РежимОтладки = Включить;
	Лог.УстановитьУровень(?(РежимОтладки, УровниЛога.Отладка, УровниЛога.Информация));
...
Функция ПолучитьПараметрыОбновленияКонфигурации(ТипКонфигурации = "Accounting",
		ВерсияРелиза = "30",
		ВерсияПлатформы = "83",
		ЗавершениеРаботыПользователейПередОбновлением = Ложь) Экспорт
	...
	ПараметрыОбновленияКонфигурации.Вставить("ЗавершениеРаботыПользователейПередОбновлением", ЗавершениеРаботыПользователейПередОбновлением);
...
Функция ПолучитьСпискиОбновленияКонфигурациии(ПараметрыОбновленияКонфигурации)
	...
	Попытка
		Соединение = Новый HTTPСоединение(НастройкиПодключения.СерверПроверкиОбновлений, , ПараметрыАутентификации.Пользователь, ПараметрыАутентификации.Пароль, , 20);
	Исключение
		Лог.Ошибка(СтрШаблон(НСтр("ru = 'Не удалось установить HTTP-соединение с сервером %1:
					|%2'"), НастройкиПодключения.СерверПроверкиОбновлений, ОписаниеОшибки()));
		Возврат Ложь;
	КонецПопытки;
	
	АдресРесурса = СтрШаблон("%1/%2/%3/%4/v8upd11.zip",
			НастройкиПодключения.ПутьКФайлуПроверкиОбновлений,
			ПараметрыОбновленияКонфигурации.ТипКонфигурации,
			ПараметрыОбновленияКонфигурации.ВерсияРелиза,
			ПараметрыОбновленияКонфигурации.ВерсияПлатформы);
	Лог.Отладка("Запрос для получения обновлений - %1", АдресРесурса);
	Попытка
	...
	Исключение
		Лог.Ошибка(СтрШаблон(НСтр("ru = 'Не удалось прочесть архив %1:
					|%2'"), ZipФайлСпискаШаблонов, ОписаниеОшибки()));
		Возврат Ложь;
	КонецПопытки;
...
Функция ПроанализироватьСпискиОбновленияКонфигурации(ПараметрыОбновленияКонфигурации)
	...
	Исключение
		Лог.Ошибка(СтрШаблон(НСтр("ru = 'Не удалось удалить временный файл %1:
					|%2'"), ФайлСпискаШаблонов, ОписаниеОшибки()));
	КонецПопытки;
...
//  Булево - Признак успешного выполнения.
//
Функция ОбновитьИнформационнуюБазу(ПараметрыОбновленияКонфигурации, 
		ПараметрыПодключения,
		ВыполнитьОтложенныеОбработчики = Истина) Экспорт
	
	Пока Истина Цикл
		
		Если ПараметрыОбновленияКонфигурации.ЗавершениеРаботыПользователейПередОбновлением = Истина Тогда
		...
		Если ВерсияДляОбновления = "ОбновленийНеТребуется" Тогда
			РезультатВыполнения = Истина;
			Прервать;
		КонецЕсли;

		Если ПараметрыОбновленияКонфигурации.ЗавершениеРаботыПользователейПередОбновлением = Ложь Тогда
			РезультатВыполнения = УстановитьБлокировкуСоединений(ПараметрыПодключения);
...
Функция ПолучитьФайлыОбновлений(ВерсияДляОбновления)
	...
	// Получаем сам файл из Интернета.
	Попытка
		Соединение = Новый HTTPСоединение(НастройкиПодключения.СерверОбновлений,
				,
				ПараметрыАутентификации.Пользователь,
				ПараметрыАутентификации.Пароль,
				,
				20);
	Исключение
		Лог.Ошибка(СтрШаблон(НСтр("ru = 'Не удалось установить HTTP-соединение с сервером %1:
					|%2'"), НастройкиПодключения.СерверОбновлений, ОписаниеОшибки()));
		Возврат Ложь;
	КонецПопытки;
	...
	Исключение
		Лог.Ошибка(СтрШаблон(НСтр("ru = 'Не удалось прочесть архив %1:
					|%2'"), ZipФайлОбновления, ОписаниеОшибки()));
		Возврат Ложь;
	КонецПопытки;
...
Функция УстановитьБлокировкуСоединений(ПараметрыПодключения) Экспорт
	...
	Если НЕ ПараметрыБлокировкиСеансов.БлокироватьСеансыИБ Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Попытка
		...
		ПараметрыАдминистрирования.АдресСервераАдминистрирования = ПараметрыПодключения.ИмяСервера1СПредприятия;
		ПараметрыАдминистрирования.ИмяАдминистратораИнформационнойБазы = ПараметрыПодключения.ИмяПользователя;
...
Функция ЗагрузитьФайлОбновленияВИнформационнуюБазу(ПараметрыПодключения, ВерсияДляОбновления)
	...
	КаталогВерсии = ВерсияДляОбновления.Каталог;
	ИмяФайлаОбновления = "1cv8.cfu";
	
	ФайлОбновления = ОбъединитьПути(КаталогВерсии, ИмяФайлаОбновления);
	
	Попытка
		УправлениеКонфигуратором.ОбновитьКонфигурациюИзФайла(ФайлОбновления, , , Истина);
	Исключение
...
Функция ВыполнитьОбновлениеКонфигурацииИнформационнойБазы(ПараметрыПодключения)
	...
	Попытка
		УправлениеКонфигуратором.ОбновитьКонфигурациюБазыДанных(Ложь, Ложь, Ложь);
	Исключение
...
//
Функция ОбеспечитьКаталог(Знач Каталог) Экспорт
...
Функция УстановитьВнешнееСоединениеСБазой(Знач ПараметрыПодключения)
	...
	ИмяCOMСоединителя = ПараметрыПодключенияКБазе.ИмяКоннектора;
	Если ПустаяСтрока(ИмяCOMСоединителя) Тогда
...
Функция УстановитьУправлениеКонфигуратором(Знач ПараметрыПодключения)
	...
	мЛог = Логирование.ПолучитьЛог("oscript.lib.v8runner");
